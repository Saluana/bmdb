...
export class TinyDB {
  ...
  constructor(pathOrOptions: any = "db.json", options: { storage?: any } = {}) {
  ...
  table<T extends Record<string, any> = any>(name: string, options: any = {}): Table<T> {
  ...
  toString(): string {
    ...
    const tablesCounts = tables.map(name => `${name}=${this.table(name).length}`);
    
    return `<TinyDB tables=[${tables.map(t => `'${t}'`).join(', ')}], ` +
