...
export class Table<T extends Record<string, any> = any> {
    ...
    constructor(
        ...
        this._queryCache = new LRUCache(
            options.cacheSize ?? Table.defaultQueryCacheCapacity
        );
            ...
            for (const document of documents) {
                ...
                if (document instanceof Document) {
                    if (String(document.docId) in table) {
                        throw new Error(
                            `Document with ID ${document.docId} already exists`
                        );
                    }
                    docId = document.docId;
                    docIds.push(docId);
            ...
            try {
                ...
                for (const [docIdStr, doc] of chunk) {
                    ...
                    if (matches) {
                        const docId = Table.documentIdClass(docIdStr);
                        chunkResults.push(
                            new Table.documentClass(doc, docId) as Document
                        );
                    }
                }
    ...
    search(cond: QueryLike<T>): Document[] {
        ...
        if (
            ...
            typeof cond === 'object' &&
            'test' in cond &&
            typeof (cond as any)._hash !== 'undefined'
        ) {
            ...
            if (results && cacheKey) {
                this._queryCache.set(
                    cacheKey,
                    results.map((doc) => this._cloneDocument(doc))
                );
            }
        ...
        if (
            ...
            const results = this._trySimpleObjectQuery(
    ...
    private _executeIndexAwareQuery(
        ...
        try {
            ...
            const skipVerification =
                plan.executionStrategy === 'index_scan' &&
    ...
    private _canSkipQueryVerification(
        query: QueryInstance<T>,
        plan: QueryPlan
    ): boolean {
        ...
        if (docIds !== undefined) {
            ...
            for (const docIdStr in table) {
                if (docIdSet.has(docIdStr)) {
                    const doc = table[docIdStr];
                    results.push(
                        this._getDocument(doc, Table.documentIdClass(docIdStr))
                    );
                }
            }
            return results;
            ...
            for (const docIdStr in table) {
                ...
                if (matches) {
                    return this._getDocument(
                        doc,
                        Table.documentIdClass(docIdStr)
                    );
                }
            }
            return null;
    ...
    updateMultiple(
        updates: Array<
            [Partial<T> | ((doc: Record<string, any>) => void), QueryLike<T>]
        >
    ): number[] {
        const updatedIds: number[] = [];
                    ...
                    if (matches) {
                        ...
                        if (!isNaN(docId) && isFinite(docId)) {
                            ...
                            try {
                                ...
                                console.warn(
                                    `Failed to update document ${docId}:`,
                                    error
                                );
                            }
                        }
    ...
    upsert(document: T | Document, cond?: QueryLike<T>): number[] {
        ...
        if (!docIds && !cond) {
            throw new Error(
                "If you don't specify a search query, you must specify a doc_id. " +
    ...
    private _cleanDocumentPool(): void {
        if (this._documentPool.size > 1000) {
            // Limit pool size
            const table = this._readTable();
    ...
    *[Symbol.iterator](): Iterator<Document> {
    ...
    _loadData(data: Record<string, any>): void {
        ...
        if (!data || typeof data !== 'object') {
            console.warn(
                `Invalid data provided to _loadData for table '${this._name}'`
            );
            return;
        }
    }
    ...
    private _generateUniqueId(
        localTable: Record<string, Record<string, any>>
    ): number {
        ...
        while (attempts < maxAttempts) {
            ...
            const candidateId =
                now * 1000 + attempts + Math.floor(Math.random() * 1000);
    ...
    private _getNextIdFromTable(
        table: Record<string, Record<string, any>> | null
    ): number {
        if (
            !table ||
            typeof table !== 'object' ||
    ...
    private _updateTable(
        ...
        if (
            this._storage.constructor.name === 'MemoryStorage' &&
            (this._storage as any).updateDocument
        ) {
            this._performDeltaUpdate(updater);
        } else if (
            (this._storage as any).writeBatch &&
            (this._storage as any).supportsFeature &&
            (this._storage as any).supportsFeature('batch')
        ) {
    ...
    private _performSelectiveUpdate(
        updater: (table: Record<string, Record<string, any>>) => void
    ): void {
        const tables = this._storage.read() || {};
        const table =
    ...
    private _createIsolatedDocument(
        docData: Record<string, any>,
        docId: number
    ): Document {
        const cowData = new CopyOnWriteObject(docData);
        const isolatedDoc = new Table.documentClass(
            cowData.getRawData(),
            docId
        ) as Document;
    ...
    toString(): string {
        const storageType = this._storage.constructor.name;
        return `<Table name='${this._name}', total=${this.length}, storage=${storageType}>`;
    }
}
