...
describe('Cross-Component Integration', () => {
    ...
    beforeEach(() => {
        const storage = new MemoryStorage();
        table = new Table<TestUser>(storage, 'regular_table', {
            enableIndexing: true,
        });
    ...
    test('should handle mixed operations across table types', () => {
        ...
        const regularResults = table.search(
            where('department').equals('Engineering')
        );
        const schemaResults = schemaTable.search(
            where('department').equals('Engineering')
        );
...
describe('Edge Cases and Error Scenarios', () => {
    ...
    beforeEach(() => {
        const storage = new MemoryStorage();
        table = new Table<TestUser>(storage, 'edge_case_test', {
            enableIndexing: true,
        });
    });
    ...
    test('should handle very large and very small numbers', () => {
        ...
        const maxResults = table.search(
            where('age').equals(Number.MAX_SAFE_INTEGER)
        );
        const minResults = table.search(
            where('age').equals(Number.MIN_SAFE_INTEGER)
        );
    ...
    test('should handle special string characters', () => {
        ...
        const unicodeResult = table.search(
            where('name').equals('Unicode: 🚀💾🔥')
        );
        ...
        for (let i = 0; i < 50; i++) {
            ...
            operations.push(async () => {
                ...
                const results = table.search(
                    where('age').greaterThan(Math.random() * 50)
                );
                table.update({ salary: Math.random() * 100000 }, undefined, [
...
describe('Data Integrity and Consistency', () => {
    ...
    test('should maintain index consistency during bulk operations', () => {
        const storage = new MemoryStorage();
        const table = new Table<TestUser>(storage, 'index_consistency', {
            enableIndexing: true,
        });
        ...
        const engineeringUsers = table.search(
            where('department').equals('Engineering')
        );
        ...
        const updatedEngineeringUsers = table.search(
            where('department').equals('Engineering')
        );
        const newEngineeringUsers = table.search(
            where('department').equals('New Engineering')
        );

        expect(updatedEngineeringUsers.length).toBeLessThan(
            initialEngineeringCount
        );
...
describe('Performance Regression Tests', () => {
    ...
    test('should scale linearly with data size', () => {
        const storage = new MemoryStorage();
        const table = new Table<TestUser>(storage, 'scaling_test', {
            enableIndexing: true,
        });
        ...
        const smallToMedium =
            scalingResults[1].duration / scalingResults[0].duration;
        const mediumToLarge =
            scalingResults[2].duration / scalingResults[1].duration;
