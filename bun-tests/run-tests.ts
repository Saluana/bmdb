...
async function runAllTests() {
    ...
    for (const testFile of testFiles) {
        ...
        if (result.passed) {
            console.log(
                `✅ ${testFile} PASSED (${result.duration.toFixed(2)}ms)`
            );
        } else {
            console.log(
                `❌ ${testFile} FAILED (${result.duration.toFixed(2)}ms)`
            );
    ...
    const memoryIssues = results.filter(
        (r) => r.output.includes('out of memory') || r.output.includes('heap')
    );
    ...
    const timeoutIssues = results.filter(
        (r) => r.output.includes('timeout') || r.output.includes('TIMEOUT')
    );
