...
async function quickTest() {
    ...
    log(
        `- Insert 1000 docs (individual): ${insertTime.toFixed(1)}ms (${(
            (1000 / insertTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    log(
        `- Insert 1000 docs (batch): ${batchInsertTime.toFixed(1)}ms (${(
            (1000 / batchInsertTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    log(
        `- Read 100 docs: ${readTime.toFixed(1)}ms (${(
            (100 / readTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    log(
        `- Update 100 docs: ${updateTime.toFixed(1)}ms (${(
            (100 / updateTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    ...
    log(
        `- Insert 1000 docs: ${insertTime.toFixed(1)}ms (${(
            (1000 / insertTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    ...
    log(
        `- Insert 1000 docs (batch): ${walBatchInsertTime.toFixed(1)}ms (${(
            (1000 / walBatchInsertTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    log(
    ...
    // Test Binary Storage
    log('## Binary Storage');
    if (existsSync('test.bin')) unlinkSync('test.bin');
    const binaryDb = new TinyDB('test.bin', { storage: BinaryStorage });

    // Individual inserts
    start = performance.now();
    for (const doc of testData) binaryDb.insert(doc);
    insertTime = performance.now() - start;
    ...
    // Batch insert
    start = performance.now();
    binaryDb.insertMultiple(testData);
    const binaryBatchInsertTime = performance.now() - start;

    start = performance.now();
    for (let i = 1; i <= 100; i++) binaryDb.search({ id: i });
    readTime = performance.now() - start;

    start = performance.now();
    for (let i = 1; i <= 100; i++) binaryDb.update({ age: 45 }, { id: i });
    updateTime = performance.now() - start;
    ...
    log(
        `- Insert 1000 docs (batch): ${binaryBatchInsertTime.toFixed(1)}ms (${(
            (1000 / binaryBatchInsertTime) *
            1000
        ).toFixed(0)} ops/sec)`
    );
    log(
    ...
    const walStorage = new WALStorage('test-tx.json', { batchSize: 50 }); // Enable batching
    ...
    log(
        `- 100 transactions: ${txTime.toFixed(1)}ms (${(
            (100 / txTime) *
            1000
        ).toFixed(0)} tx/sec)`
    );
    log(
        `- 1000 concurrent reads: ${concurrentTime.toFixed(1)}ms (${(
            (1000 / concurrentTime) *
            1000
        ).toFixed(0)} reads/sec)`
    );
